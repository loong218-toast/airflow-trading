    ...
    for family_name in ("ma", "stochastic", "lookback", "bbw"):
        ...
        for tf in tfs:
            ...
            if family_name == "ma":
                ...
                units_raw = _ordered_unique_nonneg_ints(tf_cfg.get("entry_lookback_units", []))
                if not units_raw:
                    units_raw = [0]
                ...
                for units_v in units_raw:
                    ...
                    if units_v < 0:
                        raise ValueError(
                            f"Invalid lookback units for {family_name}/{tf}: {units_raw!r}. "
                            "Use 0 to disable this timeframe, or a positive integer to enable it."
                        )
                    if units_v == 0:
                        continue
                    specs.append(
                        {
                            ...
                            "tf": tf,
                            "units": units_v,
                            "col": f"lookback_{tf}_{units_v}u",
                        }
                    )
