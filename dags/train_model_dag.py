...
from etl import train_model as etl
...
def train_lgbm_regime_model():
    ...
    def enrich_task(session_id: str, split: str, trigger: any):
        ...
        return etl.enrich_split_with_master(session_id, split)
    ...
    def curate_task(session_id, era_spec, sample_fraction):
        return etl.curate_one_era(era_spec, session_id, sample_fraction)

    @task
    def merge_task(session_id, split):
        return etl.merge_split_files(session_id, split)

    @task
    def train_task(train_path, valid_path, session_id):
        return etl.train_model(train_path, valid_path, session_id)

    @task
    def score_task(model_path, test_path, session_id):
        return etl.predict_and_score(model_path, test_path, session_id)
    ...
    def prepare_workspace(session_id: str) -> str:
        etl.cleanup_run_dir(session_id)
        return session_id

    @task
    def get_era_specs() -> list[dict]:
        return etl.discover_and_assign_era_specs()
    ...
    curation_tasks = curate_task.partial(
        ...
        sample_fraction=etl.SPLIT_SAMPLE_FRACTION,
    ).expand(
        era_spec=era_specs
    )
